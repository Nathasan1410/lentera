    ...
    class CreateFragment : Fragment() {
        ...
        private fun onButtonPress(row: Int, col: Int, button: Button) {
            ...
            // Check if the current mode is 5
            if (currentMode == 5) {
                // Reset the specific button
                ...
                Toast.makeText(
                    requireContext(),
                    "Button at ($row, $col) has been reset.",
                    Toast.LENGTH_SHORT
                ).show()
                return
            }

            // Handle mode == 2 for color update
            if (led.mode == 2) {
                if (savedColor != null) {
                ...
                // Update the mode and text for other cases
                led.mode = currentMode
                button.text = when (led.mode) {
                    0 -> "IN"
                    1 -> "OUT"
                    else -> "$row,$col" // Default text
                }
                Toast.makeText(
                    requireContext(),
